...
class TestLiveSnapshotDoesNotFinalize:
    """Tests for Phase 0: Live snapshot never writes finalized match state."""

    def test_live_snapshot_on_match_won_sets_awaiting_status(self, sqlite_db):
        """When match is won, persist_live_match_snapshot sets call_status, not completed."""
        match_id, alice_id = _seed_match(sqlite_db)
        engine = create_match(
            player_a_name="Alice",
            player_b_name="Bob",
        ...
        set_score(engine, 11, 4)  # Completes match -> match_won

        # Call live snapshot (what happens on the final point)
        persist_live_match_snapshot(match_id, engine)
        ...
        try:
            ...
            assert row.status == MatchStatus.active
            # MUST have call_status = awaiting_result_submission
            assert row.call_status == "awaiting_result_submission"
            # MUST NOT have winner
            assert row.winner is None
            assert row.winner_id is None
            # MUST NOT have completed_at
            assert row.completed_at is None
            # But score/game_scores must be written
            assert row.score == "1-0"
            assert row.game_scores == "11-4"
        finally:
            db.close()

    def test_live_snapshot_never_finalizes_match(self, sqlite_db):
        """Live snapshot on every point must never transition match to completed."""
        match_id, alice_id = _seed_match(sqlite_db)
        mm = MatchManager("Alice", "Bob")
        mm.state.player_a_id = alice_id
        mm.state.player_b_id = None

        # Score point after point
        mm._add_point("A")
        mm._add_point("A")
        # ... continue until match_won
        while mm.engine.match_status != "match_won":
            mm._add_point("A")
            if mm.engine.score_a >= 20:  # Safety break
                break

        # After each point, live snapshot is called. Verify no finalization.
        db = sqlite_db()
        try:
            row = db.query(Match).filter(Match.id == match_id).first()
            assert row.status == MatchStatus.active
            assert row.call_status in ("active", "awaiting_result_submission")
            assert row.winner is None
            assert row.winner_id is None
            assert row.completed_at is None
        finally:
    ...
    def test_live_snapshot_during_game_keeps_active(self, sqlite_db):
        """During an in-progress game, live snapshot keeps match active and normal call_status."""
        match_id, alice_id = _seed_match(sqlite_db)
        engine = create_match(
            player_a_name="Alice",
            player_b_name="Bob",
        ...
        engine.score_a = 7
        engine.score_b = 5
        engine.games_won_a = 0
        engine.match_status = "in_progress"
    ...
    def test_finalize_voice_match_succeeds_after_live_snapshot(self, sqlite_db):
        """Submit Result must still work after live snapshot was called on match_won."""
        match_id, alice_id = _seed_match(sqlite_db)
        engine = create_match(
            player_a_name="Alice",
            player_b_name="Bob",
        ...
        # Live snapshot is called on final point
        persist_live_match_snapshot(match_id, engine)

        # Submit Result (finalize) must still work
        finalize_voice_match(match_id, engine)
    ...
    def test_live_snapshot_sets_operator_note(self, sqlite_db):
        """Live snapshot must record operator_note with current game state."""
        match_id, alice_id = _seed_match(sqlite_db)
        engine = create_match(
            player_a_name="Alice",
            player_b_name="Bob",
        ...
        try:
            ...
            import json
            note = json.loads(row.operator_note)
            assert note["current_game_score"] == [8, 6]
            assert note["server"] == "A"
        finally:
            db.close()

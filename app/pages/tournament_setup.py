...
from services.tournament_engine import TournamentFactory
...
with st.sidebar:
    ...
    if ui.button("🏆 Create Tournament", key="create_tournament_btn"):
        if not tournament_name:
            ...
            try:
                # Map UI labels to model/factory types
                ...
                factory_format = 'knockout' if is_knockout else 'round-robin'
                ...
                TournamentFactory.create_tournament(factory_format, selected_players, new_tournament.id, db)
                    ...
                    if player_names:
                        ...
                        if ui.button(f"🏆 Generate {tournament_format}", key=f"gen_{tournament.id}"):
                            if len(selected_players) < 2:
                                ...
                                try:
                                    TournamentFactory.create_tournament(tournament_format, selected_players, tournament.id, db)
